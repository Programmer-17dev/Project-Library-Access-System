...
// เริ่มเซสชันและดึงค่าของ $status
session_start();
...
$sql = "
    ...
    WHERE checkin.status_place = $status
";

// ตรวจสอบว่ามีการค้นหาหรือไม่
if (!empty($search_value)) {
    $sql .= " AND (checkin.UserID LIKE '%$search_value%' 
...
$sql_total = "SELECT COUNT(*) as total FROM checkin WHERE status_place = $status";
...
if (!empty($search_value)) {
    $sql_filtered = "
        ...
        LEFT JOIN level ON checkin.Level_ID = level.Level_ID
        WHERE checkin.status_place = $status
        AND (checkin.UserID LIKE '%$search_value%' 
            OR checkin.FirstName LIKE '%$search_value%' 
            OR checkin.LastName LIKE '%$search_value%' 
            OR checkin.CheckInTime LIKE '%$search_value%' 
            OR branch.BranchName LIKE '%$search_value%' 
